...
This document defines the API contracts for the Console Todo Application based on the functional requirements.

## Core Operations

### 1. Add Task
- **Method**: `task_service.add_task(title, description)`
- **Parameters**:
...
- **Valid Options**:
  ...
  - 6: Exit
- **Error Handling**: Invalid input results in retry prompt (FR-011)

### Input Validation
- **Contract**: All user inputs must be validated before processing
- **Requirements**:
  - Empty titles rejected with validation error (FR-010)
  - Invalid task IDs handled with clear error messages (FR-009)
  - Non-numeric inputs handled gracefully (edge case from spec)
